...
class module_controller extends ctrl_module
{
     ...
     * The 'worker' methods.
     */
    public static function ListDomains($uid = 0)
    {
        global $zdbh;
        if ($uid == 0) {
            $sql = "SELECT vh_acc_fk,vh_name_vc,vh_directory_vc,vh_active_in,vh_id_pk FROM x_vhosts WHERE vh_deleted_ts IS NULL AND vh_type_in=1 ORDER BY vh_name_vc ASC";
            $numrows = $zdbh->prepare($sql);
        } else {
            $sql = "SELECT vh_acc_fk,vh_name_vc,vh_directory_vc,vh_active_in,vh_id_pk FROM x_vhosts WHERE vh_acc_fk=:uid AND vh_deleted_ts IS NULL AND vh_type_in=1 ORDER BY vh_name_vc ASC";
            $numrows = $zdbh->prepare($sql);
    ...
    public static function getDomainStatusHTML($int, $id)
    {
        global $controller;
        if ($int == 1) {
            return '<td><span style="color: green; ">' . ui_language::translate('Live') . '</span></td>'
                . '<td></td>';
        }
    ...
    public static function getCreateDomain()
    {
        $currentuser = ctrl_users::GetUserDetail();
    ...
    public static function getDomainDirsList()
    {
        $currentuser = ctrl_users::GetUserDetail();
    ...
    public static function ListDomainDirs($uid)
    {
    ...
    public static function doCreateDomain()
    {
        global $controller;
        if (!runtime_csfr::Protected()) return false;
        $currentuser = ctrl_users::GetUserDetail();
    ...
    public static function ExecuteAddDomain($uid, $domain, $destination, $autohome)
    {
        global $zdbh;
    ...
    public static function CheckCreateForErrors($domain)
    {
        global $zdbh;
    ...
    public static function IsValidDomainName($a)
    {
        if (stristr($a, '.')) {
            $part = explode(".", $a);
            foreach ($part as $check) {
    ...
    public static function CheckErrorDocument($error)
    {
    ...
    public static function doDeleteDomain()
    {
        global $controller;
        if (!runtime_csfr::Protected()) return false;
        $formvars = $controller->GetAllControllerRequests('FORM');
    ...
    public static function ExecuteDeleteDomain($id)
    {
        global $zdbh;
    ...
    public static function doConfirmDeleteDomain()
    {
        global $controller;
        if (!runtime_csfr::Protected()) return false;
        $currentuser = ctrl_users::GetUserDetail();
    ...
    public static function getisDeleteDomain()
    {
        global $controller;
    ...
    public static function getDomainUsagepChart()
    {
        global $controller;
        $currentuser = ctrl_users::GetUserDetail();
