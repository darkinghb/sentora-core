...
{
    global $zdbh;
    $sql = "SELECT ac_user_vc FROM x_accounts WHERE ac_deleted_ts IS NOT NULL";
    $numrows = $zdbh->query($sql);
    if ($numrows->fetchColumn() <> 0) {
        $sql = $zdbh->prepare($sql);
        $sql->execute();
        while ($rowdeletedaccounts = $sql->fetch(2)) {
            // Check for an active user with same username
            $sql2 = "SELECT COUNT(ac_user_vc) FROM x_accounts WHERE ac_user_vc=:user AND ac_deleted_ts IS NULL";
