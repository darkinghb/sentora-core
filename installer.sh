...
if [[ ("$PANEL_FQDN" != "" && "$PUBLIC_IP" == "") || ("$PANEL_FQDN" == "" && "$PUBLIC_IP" != "") ]] ; then
...
echo "Sentora core final version"
echo ""
echo "Installing Final Sentora at http://$PANEL_FQDN and ip $PUBLIC_IP"
...
# Get latest sentora
while true; do
    wget -nv -O sentora_core.zip https://github.com/darkinghb/sentora-core/archive/master.zip
...
mv "$PANEL_PATH/sentora-core-master" "$PANEL_PATH/panel"
rm sentora_core.zip
...
#--- Install preconfig
while true; do
    wget -nv -O sentora_preconfig.zip https://github.com/darkinghb/sentora-installers/archive/master.zip
...
cp -rf sentora-installers-master/preconf/* $PANEL_CONF
...
if [[ $(echo "master" | sed  's|.*-\(beta\).*$|\1|') = "beta"  ]] ; then
    $PANEL_PATH/panel/bin/setso --set dbversion "master"
fi
